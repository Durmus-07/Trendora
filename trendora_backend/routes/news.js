...
router.get('/', async (req, res) => {
  try {
    ...
    const region = cleanText(req.query.region).toLowerCase();
    const period = normalizePeriod(req.query.period);
    const forceRefresh = String(req.query.refresh || '').toLowerCase() === 'true';
    const page = Math.max(1, Number.parseInt(req.query.page, 10) || 1);
    const parsedLimit = Number.parseInt(req.query.limit, 10);
    const limit = Number.isFinite(parsedLimit) ? Math.min(Math.max(parsedLimit, 1), 2000) : 1500;

    const data = await getNewsData(forceRefresh, period);
    let filtered = filterByPeriod(data.items, period);

    if (requestedCategory && !['tumu', 'genel'].includes(requestedCategory)) {
      if (requestedCategory === 'son_dakika') filtered = filtered.filter(isStrictBreaking);
      else if (requestedCategory === 'gundem') filtered = filtered.filter((x) => x.category === 'gundem');
      else filtered = filtered.filter((x) => x.category === requestedCategory);
    }
    if (breakingOnly) filtered = filtered.filter(isStrictBreaking);
    if (region === 'world') filtered = filtered.filter((x) => x.region === 'world');
    if (region === 'tr') filtered = filtered.filter((x) => x.region !== 'world');

    filtered = requestedCategory && !['tumu', 'genel'].includes(requestedCategory)
      ? diversifySources(filtered)
      : balanceGeneralFeed(filtered);
    const offset = (page - 1) * limit;
    const pageItems = filtered.slice(offset, offset + limit);
    const workingSources = data.sourceResults.filter((x) => x.ok).length;

    res.json({
      success: true,
      message: 'Trendora Akıllı Haber Servisi Aktif',
      ...
      failedSources: Math.max(0, NEWS_SOURCES.length - workingSources),
      filters: { category: requestedCategory || 'genel', breakingOnly, region: region || 'all', period },
      news: pageItems,
      data: pageItems,
    });
  } catch (error) {
    console.error('[NEWS] Genel hata:', error);
    res.status(500).json({ success: false, error: 'Haberler şu anda alınamadı.', details: process.env.NODE_ENV === 'development' ? error?.message : undefined });
  }
});

router.get('/health', async (req, res) => {
  try {
    const data = await getNewsData(false, 'all');
    res.json({ success: true, service: 'Trendora Akıllı Haber Merkezi', cachedNewsCount: data.items.length, cacheUpdatedAt: new Date(data.createdAt).toISOString(), sources: data.sourceResults });
  } catch (error) {
    res.status(500).json({ success: false, error: error?.message || 'Haber servisi kontrol edilemedi.' });
  }
});
...
router.get('/sources', (req, res) => {
  res.json({
    ...
    sources: NEWS_SOURCES.map(({ name, category, url, googleQuery, priority, confidence, region, locale }) => ({
      name, category, region, priority, confidence,
      url: url || googleNewsUrl(googleQuery, '24h', locale),
    })),
  });
});

async function getNewsStatus(options = {}) {
  const data = await getNewsData(options.forceRefresh === true, 'all');
  const activeSources = data.sourceResults.filter((x) => x.ok).length;
  return {
