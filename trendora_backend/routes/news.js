...
// İlk açılışta API'nin dakikalarca boş kalmaması için önce hızlı bir ön tarama yapılır.
// En yüksek öncelikli doğrudan RSS kaynaklarıyla kısa sürede ilk veritabanı oluşturulur.
// Ardından 111 kaynağın tamamı arka arkaya taranır ve aynı veritabanı zenginleştirilir.
const BOOTSTRAP_SOURCE_COUNT = Math.max(
  8,
  Number(process.env.NEWS_BOOTSTRAP_SOURCE_COUNT || 18)
);
const BOOTSTRAP_SCAN_TIMEOUT_MS = Math.max(
  20000,
  Number(process.env.NEWS_BOOTSTRAP_SCAN_TIMEOUT_MS || 45000)
);
const BOOTSTRAP_SOURCES = NEWS_SOURCES
  .filter((source) => Boolean(source.url))
  .sort((left, right) => Number(right.priority || 0) - Number(left.priority || 0))
  .slice(0, BOOTSTRAP_SOURCE_COUNT);
...
async function refreshNewsCache(period = 'all', sources = NEWS_SOURCES, scanTimeoutMs = SCAN_TIMEOUT_MS) {
  ...
  refreshPromise = (async () => {
    ...
    try {
      ...
      console.error(
        ...
        `${completedResults.length}/${sources.length} kaynak tamamlandı. ` +
...
function createSnapshot(data, totalSources = NEWS_SOURCES.length) {
  const activeSources = data.sourceResults.filter((item) => item.ok).length;
  const completedSourceCount =
    data.completedSourceCount || data.sourceResults.length;
  ...
  return {
    ...
    totalSources,
    activeSources,
    // Yalnızca gerçekten tamamlanan kaynaklar içindeki başarısızlıkları say.
    // Ön taramada henüz denenmemiş kaynakları "başarısız" göstermiyoruz.
    failedSources: Math.max(0, completedSourceCount - activeSources),
    items: data.items,
    sourceResults: data.sourceResults,
    partial: Boolean(data.partial) || completedSourceCount < totalSources,
    completedSourceCount
  };
}

async function saveNewsSnapshot(snapshot) {
  await writeJsonAtomic(NEWS_DATABASE_FILE, snapshot);
  console.log(
    `[NEWS COLLECTOR] Veritabanı yazıldı: ${snapshot.newsCount} haber, ` +
    `${snapshot.activeSources}/${snapshot.completedSourceCount} tamamlanan kaynak`
  );
}

function hasReadyDatabase() {
  try {
    if (!fs.existsSync(NEWS_DATABASE_FILE)) return false;
    const raw = JSON.parse(fs.readFileSync(NEWS_DATABASE_FILE, 'utf8'));
    return Boolean(
      raw &&
      raw.updatedAt &&
...
async function collectAndSaveNews() {
  ...
  try {
    // Render yeniden başladığında haber API'si 2-3 dakika boyunca boş kalmasın.
    // Veritabanı hazır değilse önce doğrudan RSS kaynaklarından hızlı bir ön tarama yap.
    if (!hasReadyDatabase()) {
      console.log(
        `[NEWS COLLECTOR] Hızlı ön tarama başladı. ` +
        `Kaynak sayısı: ${BOOTSTRAP_SOURCES.length}`
      );

      const bootstrapData = await refreshNewsCache(
        'all',
        BOOTSTRAP_SOURCES,
        BOOTSTRAP_SCAN_TIMEOUT_MS
      );

      const bootstrapSnapshot = createSnapshot(
        bootstrapData,
        NEWS_SOURCES.length
      );

      if (bootstrapSnapshot.newsCount > 0) {
        await saveNewsSnapshot(bootstrapSnapshot);
        ...
        await writeCollectorStatus({
          ...
          completedAt: null,
          durationMs: Date.now() - startedAt,
          newsCount: bootstrapSnapshot.newsCount,
          totalSources: NEWS_SOURCES.length,
          activeSources: bootstrapSnapshot.activeSources,
          failedSources: bootstrapSnapshot.failedSources,
          error: null
        });

        console.log(
          `[NEWS COLLECTOR] Ön tarama tamamlandı: ` +
          `${bootstrapSnapshot.newsCount} haber. API artık hazır.`
        );
      } else {
        console.warn(
          '[NEWS COLLECTOR] Ön taramada haber alınamadı. Tam taramaya geçiliyor.'
        );
      }
    }

    console.log(
      `[NEWS COLLECTOR] Tam tarama başladı. Kaynak sayısı: ${NEWS_SOURCES.length}`
    );
    ...
    const snapshot = createSnapshot(data, NEWS_SOURCES.length);

    // Tam tarama sıfır dönerse çalışan eski veritabanını silme.
    if (snapshot.newsCount > 0) {
      await saveNewsSnapshot(snapshot);
    } else if (!hasReadyDatabase()) {
      // İlk kurulumda hiçbir kaynak cevap vermediyse yine de durum dosyasına hatayı yaz.
      throw new Error(
        'Haber kaynaklarından veri alınamadı; ilk haber veritabanı oluşturulamadı.'
      );
    } else {
      console.warn(
        '[NEWS COLLECTOR] Tam tarama sıfır haber döndürdü; mevcut veritabanı korundu.'
      );
    }
...
async function startCollector() {
  ...
  // Interval referansı açık bırakılır; collector işlemi kendi kendine kapanmaz.
}
